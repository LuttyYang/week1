...
template sudoku() {
    ...
    component range[2][9][9];
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            for (var k=0; k<2; k++) {
                range[k][i][j] = RangeProof(8);
                range[k][i][j].range[0] <== 0;
                range[k][i][j].range[1] <== 9;
            }
        }
    }
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            range[0][i][j].in <== puzzle[i][j];
            assert(range[0][i][j].out == 1);
            range[1][i][j].in <== solution[i][j];
            assert(range[1][i][j].out == 1);
